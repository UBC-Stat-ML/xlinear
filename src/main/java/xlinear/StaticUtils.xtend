...
class StaticUtils {
  ...
  def static notAVectorException() { new RuntimeException("This operation is only supported on a 1 by n or n by 1 matrix.") }
  def static notAScalarException() { new RuntimeException("This operation is only supported on a 1 by 1 matrix.") }
  def static notSymmetricPosDef() { new RuntimeException("This operation is only supported on a symmetric positive definite matrix.") }
  def static notSquare() { new RuntimeException("This operation is only supported on a square matrix.") }
  def static denseOrSparseException() { new RuntimeException("Either a SparseMatrix or DenseMatrix required.") }
  def static notBothSparseAndDense() { new RuntimeException("A matrix should not be both a SparseMatrix and a DenseMatrix") }
}